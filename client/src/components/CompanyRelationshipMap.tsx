...
function EntityNode({ company, compact = false }: { company: CompanyNetworkCompany; compact?: boolean }) {
  ...
  return (
    <div className={`rounded-lg border bg-card ${isPartner ? "border-primary" : "border-border"} ${compact ? "p-2" : "p-3"}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-foreground">{company.shortName}</p>
          <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{company.name}</p>
        </div>
        <Badge variant={isPartner ? "secondary" : "outline"} className="shrink-0 text-[10px]">{typeLabel(company.companyType)}</Badge>
      </div>
      {isPartner && <p className="mt-2 text-[11px] text-primary">{modules || "Módulos por configurar"}</p>}
      {!company.active && <p className="mt-2 text-[11px] font-medium text-[#646461]">Inativa</p>}
    </div>
  );
}
  ...
  return (
    <Card className="mt-6 border-border">
      <CardHeader className="gap-3 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <CardTitle className="flex items-center gap-2 text-base"><GitBranch className="h-4 w-4 text-primary" /> Mapa de empresas e ligações</CardTitle>
        ...
        <Select value={selectedProjectId ? String(selectedProjectId) : undefined} onValueChange={value => setSelectedProjectId(Number(value))}>
          <SelectTrigger className="w-full bg-card sm:w-[260px]"><SelectValue placeholder="Seleccione um projecto" /></SelectTrigger>
          <SelectContent>
      ...
      <CardContent>
        {!selectedProject || !network ? (
          ...
          <div className="rounded-xl border border-border bg-muted/40/70 p-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-semibold text-foreground">{selectedProject.code} — {selectedProject.name}</p>
              <Badge variant="outline" className="bg-card">{network.assigned.length} entidades atribuídas</Badge>
            </div>
            ...
            <div className="mx-auto max-w-md">
              {network.owners.length > 0 ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-center text-muted-foreground"><span className="h-5 border-l border-dashed border-border" /></div>
                </div>
              ) : (
                <div className="mb-3 flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4" /> Dono de Obra não atribuído a este projecto</div>
              )}
              ...
              {network.principals.length > 0 ? (
                      ...
                      <div key={principal.id} className="space-y-2">
                        ...
                        {partners.length > 0 && (
                          <div className="ml-4 border-l-2 border-primary pl-3">
                            <div className="mb-2 flex items-center gap-1 text-[11px] font-semibold text-primary"><Link2 className="h-3.5 w-3.5" /> EEPs subordinadas</div>
                ...
                <div className="rounded-lg border border-dashed border-border bg-card p-5 text-center text-sm text-muted-foreground">Não existem EEs ou RAPs atribuídas a este projecto.</div>
              )}

              {network.orphanPartners.length > 0 && (
                <div className="mt-4 rounded-lg border border-[#6D7A70] bg-[#EDEBEB] p-3 text-xs text-[#646461]">
            ...
            {network.otherEntities.length > 0 && (
              <div className="mt-5 border-t border-border pt-3">
                <p className="mb-2 flex items-center gap-1 text-xs font-semibold text-foreground"><Users className="h-3.5 w-3.5" /> Outras entidades atribuídas</p>
