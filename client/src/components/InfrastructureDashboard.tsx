...
export function InfrastructureDashboard({ projectId, points, futureArea, latest, readings, invoices, canConfigure, onOpenSettings, onLayoutSaved }: { projectId: number; points: InfrastructurePoint[]; futureArea?: ReadonlyArray<MapCoordinate>; latest: Record<string, any>; readings: any[]; invoices: any[]; canConfigure: boolean; onOpenSettings: () => void; onLayoutSaved: () => void }) {
  ...
  const layoutMutation = trpc.operation.updateInfrastructureMapLayout.useMutation({ onSuccess: () => { toast.success("Ajustes de posição guardados e auditados."); setAdjusting(false); setDragTarget(null); onLayoutSaved(); }, onError: error => toast.error(error.message) });
  ...
  const updateDragging = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    else setDraftFutureArea(current => current.map((point, index) => index === dragTarget.index ? coordinate : point));
  };
  const startDrag = (event: React.PointerEvent<HTMLButtonElement>, target: { kind: "marker"; id: number } | { kind: "future"; index: number }) => {
    if (!adjusting) return;
  ...
  const saveAdjustment = () => layoutMutation.mutate({ projectId, markers: draftMarkers, futureArea: draftFutureArea });
