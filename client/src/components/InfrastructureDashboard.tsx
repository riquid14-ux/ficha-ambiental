...
export function InfrastructureDashboard({ projectId, points, futureArea, mapLabels, latest, readings, invoices, canConfigure, onOpenSettings }: { projectId: number; points: InfrastructurePoint[]; futureArea?: ReadonlyArray<MapCoordinate>; mapLabels?: MapLabels; latest: Record<string, any>; readings: any[]; invoices: any[]; canConfigure: boolean; onOpenSettings: () => void }) {
  ...
  return <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
      ...
      {canConfigure && <div className="flex flex-wrap gap-2"><Button variant="outline" className="border-slate-300" onClick={onOpenSettings}><Settings2 className="mr-2 size-4" />Definir infraestrutura</Button></div>}
    </div>
    <div style={{ aspectRatio: "3 / 1", minHeight: 0 }} className="relative overflow-hidden bg-slate-950">
        ...
        <div style={{ clipPath: futureAreaClip([...resolvedFutureArea]) }} className="absolute inset-0 border border-fuchsia-950/80 bg-fuchsia-800/52 shadow-[inset_0_0_60px_rgba(112,26,117,0.3)]">
        ...
        <span style={{ left: `${resolvedLabels.futureArea.xPercent}%`, top: `${resolvedLabels.futureArea.yPercent}%` }} className="absolute z-20 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded-full border border-fuchsia-100/80 bg-slate-950/85 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-fuchsia-100 shadow-lg">Edifícios futuros · planeamento</span>
        {visiblePoints.filter(point => !point.isFuture).map(point => {
          const Icon = SYSTEM_ICONS[point.systemType] || MapPin;
          return <button key={point.id} type="button" aria-label={`${t("Abrir informação de")} ${pointTitle(point)}`} onClick={() => setSelected(point)} style={{ left: `${point.xPercent}%`, top: `${point.yPercent}%` }} className="group absolute z-20 grid size-8 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-2 border-yellow-50 bg-yellow-300 text-slate-950 shadow-[0_0_0_5px_rgba(255,255,255,0.24)] transition hover:scale-110 hover:bg-yellow-200 focus:outline-none focus:ring-4 focus:ring-yellow-200"><Icon className="size-3.5" /><span className="pointer-events-none absolute left-1/2 top-full mt-2 w-max max-w-48 -translate-x-1/2 rounded-md bg-slate-950/90 px-2 py-1 text-[10px] font-medium text-white opacity-0 shadow-lg transition group-hover:opacity-100 group-focus:opacity-100">{pointTitle(point)}</span></button>;
        })}
        <div className="absolute bottom-4 left-4 flex flex-wrap gap-2"><span className="rounded-full bg-slate-950/80 px-3 py-1.5 text-xs text-white backdrop-blur">{visiblePoints.filter(point => !point.isFuture).length} {t("Marcadores a configurar")}</span><span className="rounded-full bg-violet-900/80 px-3 py-1.5 text-xs text-violet-100 backdrop-blur">{t("Edifícios futuros · planeamento")}</span></div>
        <div style={{ left: `${resolvedLabels.guidance.xPercent}%`, top: `${resolvedLabels.guidance.yPercent}%` }} className="absolute z-10 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded-full border border-white/30 bg-slate-950/78 px-4 py-2 text-xs font-medium text-white shadow-lg backdrop-blur">Clique num marcador para abrir o respetivo contexto técnico.</div>
    </div>
