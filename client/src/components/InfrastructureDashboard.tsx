...
export function InfrastructureDashboard({ points, latest, readings, canConfigure, onOpenSettings }: { points: InfrastructurePoint[]; latest: Record<string, any>; readings: any[]; canConfigure: boolean; onOpenSettings: () => void }) {
  ...
  return <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="relative min-h-[520px] overflow-hidden bg-slate-950">
        ...
        <div className="absolute right-4 top-4 max-w-xs rounded-2xl border border-white/20 bg-slate-950/75 px-4 py-3 text-white shadow-lg backdrop-blur"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-teal-200">Infraestrutura interativa</p><p className="mt-1 text-sm leading-5 text-slate-100">Clique diretamente nos marcadores para abrir o respetivo contexto técnico.</p></div>
    </div>
