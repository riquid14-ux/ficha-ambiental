    ...
    <section className={`stand-page-header relative isolate overflow-hidden ${toneClasses[tone]}`}>
      ...
      <div className="relative flex flex-col gap-5 p-5 sm:p-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 max-w-3xl">
          {eyebrow && <p className={`stand-kicker ${isOperations ? "text-primary" : "text-primary"}`}>{eyebrow}</p>}
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <h1 className="stand-page-title">{title}</h1>
            {context && <Badge variant="outline" className={isOperations ? "border-white/20 bg-card/10 text-primary" : "border-primary/20 bg-primary/5 text-primary"}>{context}</Badge>}
          </div>
          {description && <p className={`mt-2 max-w-2xl text-sm leading-6 ${isOperations ? "text-white/75" : "text-muted-foreground"}`}>{description}</p>}
