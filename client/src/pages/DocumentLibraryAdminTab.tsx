...
function DocumentScopeFields({ value, onChange, projects, prefix }: { value: DocumentScope; onChange: (value: DocumentScope) => void; projects: Array<{ id: number; code: string; name: string }>; prefix: string }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-[#0A3638] bg-[#0A3638]/50 p-3 md:col-span-2 xl:col-span-3">
      <div className="flex items-start gap-3">
        ...
        <div><Label htmlFor={`${prefix}-all-projects`} className="font-semibold text-foreground">Aplicar a todos os projetos</Label><p className="mt-1 text-xs text-muted-foreground">O documento ficará disponível em cada projeto individual, incluindo projetos adicionados no futuro.</p></div>
      </div>
      {!value.appliesToAllProjects && <div className="space-y-2 border-t border-[#0A3638] pt-3"><p className="text-sm font-medium text-foreground">Selecionar projetos aplicáveis</p><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{projects.map(project => <label key={project.id} className="flex cursor-pointer items-start gap-2 rounded-md border bg-card p-2.5 text-sm"><Checkbox checked={value.projectIds.includes(project.id)} onCheckedChange={checked => toggleProject(project.id, checked === true)} /><span><strong>{project.code}</strong><span className="block text-xs text-muted-foreground">{project.name}</span></span></label>)}</div>{projects.length === 0 && <p className="text-xs text-muted-foreground">Não existem projetos operacionais disponíveis.</p>}</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <Card className="border-primary">
        <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><BookOpen className="h-5 w-5 text-primary" />Configuração da Documentação</CardTitle><p className="text-sm text-muted-foreground">Defina o título, descrição, língua, tópico, visibilidade e PDF. Marque DCAPE, PGA e outros documentos-base para os destacar na Central de Documentos do Projeto.</p></CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="flex items-start gap-3 rounded-lg border border-[#6D7A70] bg-[#EDEBEB] p-3 md:col-span-2 xl:col-span-3"><Checkbox id="document-central" checked={form.isProjectCentral} onCheckedChange={checked => setForm(current => ({ ...current, isProjectCentral: checked === true, isMandatoryRead: checked === true ? current.isMandatoryRead : false, centralOrder: checked === true ? current.centralOrder : 0 }))} /><div className="grid gap-1"><Label htmlFor="document-central" className="flex items-center gap-1.5 font-semibold text-foreground"><ShieldCheck className="h-4 w-4 text-[#646461]" />Destacar na Central de Documentos do Projeto</Label><p className="text-xs leading-relaxed text-muted-foreground">Use para DCAPE, PGA e outros documentos que estabelecem as regras de projecto.</p></div></div>
          {form.isProjectCentral && <><div className="flex items-start gap-3 rounded-lg border bg-muted/40 p-3"><Checkbox id="document-mandatory" checked={form.isMandatoryRead} onCheckedChange={checked => setForm(current => ({ ...current, isMandatoryRead: checked === true }))} /><div className="grid gap-1"><Label htmlFor="document-mandatory" className="flex items-center gap-1.5 font-semibold"><ClipboardCheck className="h-4 w-4 text-[#646461]" />Assinalar como leitura obrigatória</Label><p className="text-xs text-muted-foreground">É apresentado com aviso reforçado na central.</p></div></div><div className="space-y-2"><Label htmlFor="central-order">Ordem na Central</Label><Input id="central-order" type="number" min={0} max={999} value={form.centralOrder} onChange={event => setForm(current => ({ ...current, centralOrder: Math.max(0, Math.min(999, Number(event.target.value) || 0)) }))} /><p className="text-xs text-muted-foreground">0 aparece primeiro.</p></div></>}
