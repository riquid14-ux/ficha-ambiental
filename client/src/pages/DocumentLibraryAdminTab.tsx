...
type LibraryItem = {
  ...
  filename: string; fileSize: number; status: Status; isProjectCentral: number; isMandatoryRead: number; centralOrder: number; createdByName: string; createdAt: Date | string;
};

const initialForm = { topic: "obrigacoes_ambientais" as Topic, subtopic: "", title: "", language: "Português (Portugal)", description: "", status: "draft" as "draft" | "published", isProjectCentral: false, isMandatoryRead: false, centralOrder: 0 };
...
export default function DocumentLibraryAdminTab() {
  ...
  const [editForm, setEditForm] = useState({ topic: "obrigacoes_ambientais" as Topic, subtopic: "", title: "", language: "", description: "", status: "draft" as Status, isProjectCentral: false, isMandatoryRead: false, centralOrder: 0 });
  ...
  const openEdit = (document: LibraryItem) => {
    setEditing(document);
    setEditForm({ topic: document.topic, subtopic: document.subtopic || "", title: document.title, language: document.language, description: document.description || "", status: document.status, isProjectCentral: document.isProjectCentral === 1, isMandatoryRead: document.isMandatoryRead === 1, centralOrder: document.centralOrder || 0 });
  };
