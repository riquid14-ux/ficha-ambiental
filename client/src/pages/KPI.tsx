...
const COLORS = ["#22c55e", "#0A3638", "#0A3638", "#ef4444", "#0A3638", "#00C159", "#ec4899", "#84cc16"];
      ...
      <div className="space-y-6">
        <StandPageHeader
          ...
          actions={<>
            ...
            <Button variant="secondary" size="sm" className="min-h-9 border border-white/15 bg-card/10 text-white hover:bg-card/20 hover:text-white" disabled={importExcelMutation.isPending || !effectiveCompanyId} onClick={() => importInputRef.current?.click()}><Upload className="mr-1.5 h-4 w-4" /> {importExcelMutation.isPending ? "A importar..." : "Importar Excel"}</Button>
            {user?.role === "admin" && <Button variant="secondary" size="sm" className="min-h-9 border border-white/15 bg-card/10 text-white hover:bg-card/20 hover:text-white" onClick={() => setShowSettings(!showSettings)}><Settings className="mr-1.5 h-4 w-4" /> {t("Definições")}</Button>}
          </>}
        />
        ...
        {showSettings && user?.role === "admin" && (
          <Card className="overflow-hidden rounded-2xl border-[#0A3638]/18 bg-card shadow-sm">
            <CardHeader className="border-b border-[#0A3638]/18 bg-[#F4F4FF] pb-4"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0A3638] text-white"><Settings className="h-4 w-4" /></span><CardTitle className="text-base">Configuração de métricas KPI</CardTitle></div><p className="pt-1 text-xs leading-5 text-muted-foreground">Crie métricas manuais como viaturas em obra, defina unidade, categoria, meta e posição. Todas as alterações ficam auditadas.</p></CardHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
            ...
            <Card className="overflow-hidden rounded-2xl border-border/80 bg-card shadow-sm"><CardHeader className="border-b border-border/70 bg-muted/20 pb-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="stand-kicker text-primary">COBERTURA DE REPORTE</p><CardTitle className="mt-1 text-base">{t("Matriz de Submissão")}</CardTitle><p className="mt-1 text-xs leading-5 text-muted-foreground">Acompanhe a entrega consolidada por empresa e semana de reporte.</p></div><StandStatusBadge label={`${matrixWeeks.length} semanas visíveis`} tone="info" /></div></CardHeader><CardContent className="p-0">
              {matrixQuery.isLoading ? <div className="flex items-center justify-center gap-2 py-6 text-sm text-muted-foreground"><Activity className="size-4 animate-spin" />A carregar submissões consolidadas...</div> : matrixQuery.isError ? <p className="py-4 text-center text-sm text-destructive">Não foi possível carregar a matriz de submissões.</p> : matrixWeeks.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">{ t("Sem submissões.") }</p> : (
                <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-xs"><caption className="sr-only">{t("Matriz de Submissão")}</caption><thead className="bg-muted/35"><tr className="border-b border-border/70"><th scope="col" className="p-3 text-left font-semibold text-muted-foreground">{t("Empresa")}</th>{matrixWeeks.map(w => <th scope="col" key={`${w.weekYear}-${w.weekNumber}`} className="p-3 text-center font-semibold text-muted-foreground">S{w.weekNumber}</th>)}</tr></thead><tbody>{matrixCompanies.map(([id, name]) => (
                  <tr key={id} className="border-b border-border/60 transition-colors hover:bg-primary/[0.035]"><th scope="row" className="p-3 text-left font-semibold text-foreground">{name}</th>{matrixWeeks.map(w => { const s = matrixData.find((x: any) => x.companyId === id && x.weekNumber === w.weekNumber && x.weekYear === w.weekYear); return <td key={`${w.weekYear}-${w.weekNumber}`} className="p-3 text-center">{s ? <><CheckCircle aria-hidden className="mx-auto h-4 w-4 text-primary" /><span className="sr-only">Submetido</span></> : <><XCircle aria-hidden className="mx-auto h-4 w-4 text-destructive/55" /><span className="sr-only">Sem submissão</span></>}</td>; })}</tr>
                ))}</tbody></table></div>
              )}
            </CardContent></Card>
          </TabsContent>
            ...
            <Card className="overflow-hidden rounded-2xl border-border/80 bg-card shadow-sm">
              ...
              <CardContent>
                ...
                {affluentWaterMetrics.length > 0 && (
                  <section className="mt-6 rounded-2xl border border-[#0A3638]/18 bg-[#F4F4FF] p-4">
                    <div className="mb-3 flex items-start gap-2"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#0A3638] text-white"><Droplets className="h-4 w-4" /></span><div><h3 className="text-sm font-semibold text-foreground">Águas Afluentes</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Registe separadamente as águas residuais e operações associadas ao período seleccionado.</p></div></div>
                ...
                {formStep === categories.length - 1 && calculatedMetrics.length > 0 && (
                  <div className="mt-5 rounded-2xl border border-primary/24 bg-primary/[0.045] p-4">
                    <div className="mb-2 flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-[#0A3638]"><Activity className="h-3.5 w-3.5" /></span><p className="text-xs font-semibold text-foreground">{t("Valores Calculados Automaticamente")}</p></div>
                    {calculatedMetrics.map((m: any) => {
                      ...
                      return <div key={m.id} className="flex items-center justify-between gap-3 border-t border-primary/20 py-2 text-sm"><span className="text-muted-foreground">{m.name}</span><strong className="tabular-nums text-foreground">{cv.toFixed(1)} {m.unit}</strong></div>;
                    })}
                  </div>
                )}
                ...
                <div className="mt-5 flex flex-wrap justify-between gap-2 border-t border-border/70 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {formStep < categories.length - 1 ? (
                      ...
                      <Button onClick={handleSubmit} disabled={submitMutation.isPending || correctMutation.isPending} className="min-h-10 rounded-xl bg-primary hover:bg-primary/85"><Send className="mr-2 h-4 w-4" />{submitMutation.isPending || correctMutation.isPending ? "A guardar..." : existingSubmission && existingSubmission.status !== "draft" ? "Registar correção" : isPartner ? "Submeter contributo parcial" : "Submeter KPIs"}</Button>
                    )}
                  </div>
                </div>
              </CardContent>
          ...
          {isAdminOrDO && (
            <TabsContent value="dashboard" className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 border-b border-border/70 pb-3">
                ...
                {dashPage === 0 && <>
                <ChartCard title={t("Consumo Combustível (L)")}><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Bar dataKey="fuel" fill="#0A3638" /></BarChart></ChartCard>
                <ChartCard title="Eletricidade (kWh)"><AreaChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Area type="monotone" dataKey="energy" fill="#00C159" stroke="#0A3638" fillOpacity={0.3} /></AreaChart></ChartCard>
                ...
                <ChartCard title="CO2 Acumulado"><AreaChart data={chartData.reduce((acc: any[], d: any, i: number) => { const prev = acc[i - 1]?.cumFuel || 0; acc.push({ ...d, cumFuel: prev + (d.fuel || 0) }); return acc; }, [])}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Area type="monotone" dataKey="cumFuel" fill="#0A3638" stroke="#0A3638" fillOpacity={0.2} /></AreaChart></ChartCard>
                </>}
                {dashPage === 1 && <>
                <ChartCard title={t("Consumo Água (L)")}><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Bar dataKey="water" fill="#00C159" /></BarChart></ChartCard>
                <ChartCard title={t("Água Acumulada")}><AreaChart data={chartData.reduce((acc: any[], d: any, i: number) => { const prev = acc[i - 1]?.cumWater || 0; acc.push({ ...d, cumWater: prev + (d.water || 0) }); return acc; }, [])}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Area type="monotone" dataKey="cumWater" fill="#00C159" stroke="#0A3638" fillOpacity={0.2} /></AreaChart></ChartCard>
                ...
                {waterMetricGroups.affluent.length > 0 && <div className="rounded-2xl border border-[#0A3638]/18 bg-[#F4F4FF] p-4 md:col-span-2"><div className="mb-3 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0A3638] text-white"><Droplets className="h-4 w-4" /></span><div><h3 className="text-sm font-semibold text-foreground">Águas Afluentes</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Indicadores registados separadamente do consumo de água.</p></div></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{waterMetricGroups.affluent.map((metric: any) => <div key={metric.id} className="rounded-xl border border-[#0A3638]/18 bg-card p-3"><p className="text-xs text-muted-foreground">{metric.name}</p><p className="mt-1 text-lg font-semibold tabular-nums text-foreground">{formatNumber(totals[metric.id] || 0)} <span className="text-xs font-medium text-muted-foreground">{metric.unit}</span></p></div>)}</div></div>}
                </>}
                {dashPage === 2 && <>
                <ChartCard title="Trabalhadores em Obra"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Line type="monotone" dataKey="workforce" stroke="#0A3638" strokeWidth={2} /></LineChart></ChartCard>
                <ChartCard title="Trabalhadores vs Horas"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Legend /><Line type="monotone" dataKey="workforce" stroke="#0A3638" name={t("Trabalhadores")} /><Line type="monotone" dataKey={`m_${findMetric("workforce", "horas")?.id || 0}`} stroke="#00C159" name={t("Horas")} /></LineChart></ChartCard>
                <ChartCard title="Transporte"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Bar dataKey="transport" fill="#00C159" /></BarChart></ChartCard>
          ...
          {user?.role === "admin" && (
            <TabsContent value="metas" className="space-y-4">
              ...
              {editingTarget && (
                <Card className="rounded-2xl border-primary/20 bg-primary/[0.045] shadow-sm dark:bg-primary/10"><CardContent className="space-y-3 p-4 sm:p-5">
              ...
              {targets.length === 0 ? <p className="text-sm text-muted-foreground text-center py-6">Sem metas para {targetYear}.</p> : targets.map((t: any) => {
                ...
                return (
                  <Card key={t.id} className={`overflow-hidden rounded-2xl border shadow-sm ${isGood ? "border-primary/25 bg-primary/[0.025] dark:bg-primary/10" : "border-rose-500/25 bg-rose-500/[0.025] dark:bg-rose-500/10"}`}><CardContent className="flex flex-wrap items-center gap-4 p-4 sm:p-5">
                    ...
                    <div className="w-28"><div className="h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${isGood ? "bg-primary" : "bg-rose-600"}`} style={{ width: `${Math.min(progress, 100)}%` }} /></div><p className="mt-1 text-center text-xs font-medium text-muted-foreground">{progress.toFixed(0)}%</p></div>
