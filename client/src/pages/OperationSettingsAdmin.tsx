...
import { Activity, BadgeEuro, Factory, Gauge, Leaf, MapPin, Save, Settings2, Sparkles, Waves } from "lucide-react";
...
export default function OperationSettingsAdmin() {
  ...
  return <div className="space-y-5">
    ...
    <Tabs defaultValue="eficiencia" className="space-y-4"><TabsList className="flex h-auto w-full flex-wrap justify-start gap-1 rounded-xl bg-slate-100 p-1">{sections.map(section => { const Icon = section.icon; return <TabsTrigger key={section.id} value={section.id} className="gap-2 rounded-lg px-4"><Icon className="size-4" />{section.label}</TabsTrigger>; })}<TabsTrigger value="sistema" className="gap-2 rounded-lg px-4"><Factory className="size-4" />Sistema-base</TabsTrigger><TabsTrigger value="infraestrutura" className="gap-2 rounded-lg px-4"><MapPin className="size-4" />Infraestrutura</TabsTrigger></TabsList>{sections.map(section => <TabsContent key={section.id} value={section.id}><SettingsSection title={section.title} description={section.description} icon={section.icon}>{section.fields.map(field => <SettingField key={field.key} field={field} value={draft[field.key]} onChange={value => setDraft(current => ({ ...current, [field.key]: value }))} />)}</SettingsSection></TabsContent>)}<TabsContent value="sistema"><SettingsSection title="Estratégia operacional de referência" description="Define o sistema dominante usado para enquadrar cenários e comparações de desempenho." icon={Factory}><div className="rounded-xl border border-slate-200 bg-slate-50 p-4 md:col-span-2"><Label>Estratégia-base de arrefecimento</Label><Select value={coolingStrategyBaseline} onValueChange={setCoolingStrategyBaseline}><SelectTrigger className="mt-2 max-w-md bg-white"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(strategyLabels).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select><p className="mt-2 text-xs text-muted-foreground">Esta escolha não altera dados medidos. É usada apenas como contexto e pressuposto de cenários.</p></div></SettingsSection></TabsContent><TabsContent value="infraestrutura"><InfrastructureSettingsSection projectId={projectId} /></TabsContent></Tabs>
