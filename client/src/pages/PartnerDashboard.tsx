...
export default function PartnerDashboard() {
  ...
  const workerMetric = findMetric("workforce", "trabalhadores em obra") || findMetric("workforce", "trabalhadores");
  ...
  if (user?.role !== "ee") return <AppLayout><div className="p-8 text-center text-muted-foreground">Dashboard exclusivo das Entidades Executantes.</div></AppLayout>;
  if (!activeProject) return <AppLayout><div className="p-8 text-center text-muted-foreground">Seleccione um projecto.</div></AppLayout>;

  return (
    <AppLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold">Dashboard Parceiros</h1><p className="text-sm text-muted-foreground">{activeProject.code} · EE e EEP subordinadas</p></div><div className="flex gap-3"><div><Label>Ano</Label><Input className="w-24" type="number" value={year} onChange={event => setYear(Number(event.target.value))} /></div><div className="min-w-48"><Label>Entidade</Label><Select value={companyFilter} onValueChange={setCompanyFilter}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todos — EE + EEP</SelectItem>{entityRows.map(entity => <SelectItem key={entity.id} value={String(entity.id)}>{entity.entityType === "ee" ? "EE" : "EEP"} · {entity.shortName}</SelectItem>)}</SelectContent></Select></div></div></div>

        <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle className="flex items-center gap-2 text-base"><Users className="h-4 w-4" />Matriz de submissão KPI</CardTitle><div className="flex items-center gap-2 text-xs"><span>Semanas</span><Input className="h-8 w-16" type="number" min={1} max={53} value={startWeek} onChange={event => setStartWeek(Math.max(1, Math.min(Number(event.target.value), endWeek)))} /><span>a</span><Input className="h-8 w-16" type="number" min={1} max={53} value={endWeek} onChange={event => setEndWeek(Math.min(53, Math.max(Number(event.target.value), startWeek)))} /></div></div></CardHeader><CardContent className="overflow-x-auto"><table className="w-full border-collapse text-xs"><thead><tr><th className="sticky left-0 bg-background p-2 text-left">Entidade</th>{weeks.map(week => <th key={week} className="min-w-20 p-2 text-center">S{week}</th>)}</tr></thead><tbody>{entityRows.filter(entity => Boolean(entity.allowKpi)).map(entity => <tr key={entity.id} className="border-t"><td className="sticky left-0 bg-background p-2 font-medium">{entity.shortName}</td>{weeks.map(week => { const ok = submitted.has(`${entity.id}-${week}`); return <td key={week} className="p-2 text-center"><span title={ok ? "Submetido" : "Não submetido"} className={`inline-flex h-7 w-7 items-center justify-center rounded-full ${ok ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>{ok ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}</span></td>; })}</tr>)}</tbody></table></CardContent></Card>
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <KpiChartCard id="workforce" title="Trabalhadores em obra"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><EmptyAwareTooltip /><Line type="monotone" dataKey={workerMetric ? `m_${workerMetric.id}` : "workforce"} name="Trabalhadores" stroke="#7c3aed" strokeWidth={2} dot={false} /></LineChart></KpiChartCard>
